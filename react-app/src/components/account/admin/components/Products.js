...
const Products = () => {
    ...
    return <>
        ...
        <div className="title">
            <i></i>
            <h6><i className="icons-shopping_bag"></i>محصولات من</h6>
                    ...
                    {(posts.have_posts) ?
                        currentPageData.map((val, i) => {
                            return <tr key={i}>
                                ...
                                <td>{(parseInt(val.post_price) > 0) ?
                                    <p>{new Intl.NumberFormat().format(parseInt(val.post_price))} <small>تومان</small></p>
                                    : <p>رایگان</p>
                                }</td>

                                <td><p>{val.post_discount} <small>%</small></p></td>

                                <td><p>{val.post_selling}</p></td>

                                <td>{val.post_view}</td>
                            </tr>;
                        }) :
                        <tr>
                            <td colSpan={9} className='posts-not-found'><p>چیزی برای نمایش وجود ندارد!</p></td>
                        </tr>}
                </tbody>
            </table>
